...
public class VillagerWitnessEvent {
    ...
    private static int getBaseExposure(Player player) {
        ...
        if (vampireLevel >= 10 && lordLevel == 0) return 10;
        else if (lordLevel > 0) return 14;
        else return 8;
    }
    ...
    public static void handleWitness(Player player) {
        ...
        if (!observers.isEmpty()) {
            int baseExposure = getBaseExposure(player);
            int count = Math.min(observers.size(), maxObservers);
            int deltaExposure = (int) Math.round(baseExposure * (1 - Math.exp(-0.35 * count)));
            if (inFog) {
            ...
            if (!inFog && !Helper.isEntityInVampireBiome(player)) {
                ...
                for (LivingEntity e : observers) {
                    if (processed >= maxObservers) break;
                    if (e.getPersistentData().getBoolean(VO)) {
                        panicBase += e.isSleeping() ? 1 : 2;
                        processed++;
                    }
                }

                if (panicBase > 0) {
                    if (data == null) data = MasqueradeVillageData.get(level);
